...
- **외부 의존성 0개.** 표준 라이브러리 + macOS 내장 프레임워크(CryptoKit, Vision, AVFoundation, Security, SwiftUI)만 사용한다.
...
2. **QR 추가** 버튼
3. 방법 둘 중 하나:
   - **카메라 탭** — Google Authenticator 앱 → 우상단 메뉴 → "계정 내보내기"에서 표시되는 QR을 맥북 카메라에 비춘다 (한 번에 여러 계정 일괄 가져오기 가능)
   - **이미지 파일 탭** — QR이 포함된 스크린샷/이미지를 드롭하거나 클릭해서 선택
...
│   └── CameraQRScanner.swift    # AVFoundation
└── Authenticator/             # SwiftUI 메뉴바 앱
...
- **카메라 권한** — 첫 사용 시 시스템 권한 요청. 거부했다면 시스템 설정 > 개인정보 보호 및 보안 > 카메라에서 허용.
